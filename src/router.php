    ...
    public function run()
    {
        ...
        if (isset($_GET['route'])) {
            if ('articleControllers' === $route && $action) {
                ...
                }elseif ('updateArticle' === $action && isset($_GET['id'])) {
                    return $articleController->updateArticle($_GET['id']);
                }elseif('deleteArticle' === $action && isset($_GET['id'])){
                    return $articleController->deleteArticle($_GET['id']);
                }
            }
        }else{
            require_once 'vue/accueil.php';
